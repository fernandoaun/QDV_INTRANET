...
def _run_exclusive(app: Any, fn: Callable[[], None]) -> None:
    ...
    try:
        try:
            ...
            log.debug("Respaldo BD: otro proceso ya está ejecutando el ciclo.")
            return
...
def _run_backup_cycle(app: Any) -> None:
    ...
    if out.get("skipped"):
        ...
        return
    if out.get("ok"):
        log.info("Respaldo BD automático: %s", out.get("filename") or out.get("message"))
    else:
        log.warning("Respaldo BD automático falló: %s", out.get("message") or "")
...
def _scheduler_loop(app: Any) -> None:
    ...
    while True:
        try:
            ...
            log.exception("Fallo en ciclo automático de respaldo de BD")
